...
public class AbilitiesScript : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        centreButton.OnInteract += delegate()
        {
            submitButtonPressed();
            return false;
        };
    ...
    private void submitButtonPressed()
    {
        ...
        {
            log(string.Format("Strike! Expected '{0}' but '{1}' was selected instead", _abilityNames[0],
                _abilityNames[CurrentSelectedIndex]));
...
#pragma warning disable 414
    private readonly string TwitchHelpMessage =
        @"!{0} select <answer> [Selects the specified answer] | !{0} left/right <amount (optional)> [Presses the left or right arrow the amount specified]| !{0} submit/enter [Submits the current input] | !{0} submit/enter <answer> [Submits the specified answer]";
#pragma warning restore 414
    ...
    private IEnumerator ProcessTwitchCommand(string command)
    {
                ...
                RegexOptions.IgnoreCase | RegexOptions.CultureInvariant))
        {
            ...
            {
                yield return "sendtochaterror!f The specified number of times to press '" + parameters[1] +
                             "' is invalid!";
                yield break;
            }
            ...
            {
                yield return "sendtochaterror The specified number of times to press '" + parameters[1] +
                             "' is less than 1!";
                yield break;
            }
        ...
        if (Regex.IsMatch(parameters[0], @"^\s*(submit|enter)\s*$",
                RegexOptions.IgnoreCase | RegexOptions.CultureInvariant))
        {
            yield return null;
